 ...
 * @return fileState_t 
 */
fileState_t handleData(const char *name, const int size)
{
    FILE *inputFile = NULL;
    ...
    {
        if (processData(&buf,outputFile,&outputData, size) != fileOK)
        {
            return fileError;
        }
 ...
 * @return fileState_t 
 */
fileState_t processData(queue_t *buf, FILE *outputFile, outputData_t *outputData, const int size)
{
    putIntoQueue(buf,outputData->dataIn, size);
